...
class ArticleDetailViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    @objc func showArticle() {
        ...
        _ = executeTask(url_server!, requestParam) { (data, response, error) in
            if error == nil {
                if data != nil {
                    // 將輸入資料列印出來除錯用
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        _ = executeTask(url_image!, requestParam) { (data, response, error) in
            if error == nil {
                if data != nil {
    ...
    func cancelFavorite() {
        ...
        requestParam["articleId"] = articleDetail.articleId
        requestParam["loginUserId"] = COMM_USER_ID
        let task = executeTask(self.url_server!, requestParam) { (data, response, error) in
            if error == nil {
                if data != nil {
                    if let result = String(data: data!, encoding: .utf8) {
                        if Int(result) != nil {
    ...
    @IBAction func loginAction(_ sender: UIButton) {
        let loginVC = UIStoryboard(name: "User", bundle: nil).instantiateViewController(withIdentifier: "UserDataSetupVC")
